...
<div x-data="{
    ...
    <form method="POST" action="{{ route('admin.pelamar.notify', $pelamar) }}" class="mt-4 flex gap-2">
        ...
        <x-ui.button type="submit" name="channel" value="whatsapp" @click="if(!confirm('Buka WhatsApp untuk mengirim pesan ini?')) $event.preventDefault()"
        ...
        <x-ui.button type="submit" name="channel" value="email" @click="if(!confirm('Buka aplikasi email untuk mengirim pesan ini?')) $event.preventDefault()"
